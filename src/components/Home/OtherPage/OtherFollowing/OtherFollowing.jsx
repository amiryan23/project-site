import s from './OtherFollowing.module.scss'
...
const unfollowMutation = useUnfollowMutation()
 ...
 return () => {
  ...
  const followingArray = selectedUser 
  ?
  selectedUser.userData?.following?.map(m => 
  ...
  : ""
	...
	return (
		...
		<div className={s.content1}>
			...
			<span className={s.item1}>{t('Following')} </span>
			<span className={s.item2}>{selectedUser.userData?.following?.length}</span>
		</div>	
		<div className={s.content2}>
			{selectedUser && followingArray}
		</div>
		</div>
		)
}
