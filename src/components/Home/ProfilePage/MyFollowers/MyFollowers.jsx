import s from './MyFollowers.module.scss'
...
// }) : null;
  ...
  const followersArray = thisUser?.userData?.followers?.length > 0
  ?
  thisUser.userData?.followers?.map(m => 
  ...
  : <div className={s.noFollowers}>{t('NoFolowers')}</div>
	...
	return (
		...
		<div className={s.content1}>
			...
			<span className={s.item2}>{thisUser?.userData?.followers?.length}</span>
		</div>	
		<div className={s.content2}>
			{thisUser && followersArray}
		</div>
		</div>
		)
}
